...
public class BidController {
    ...
    @GetMapping
    public ResponseEntity<List<BidShowDto>> getAllBids(){
        logger.info("http get request invoked to: '/bid'");
        return new ResponseEntity<>(bidService.getAllBids(), HttpStatus.OK);
    }
    @PostMapping
    public ResponseEntity<Void> addBid(@RequestBody @Valid BidCreateDto bidCreateDto, BindingResult bindingResult){
        if(!bindingResult.hasErrors() && bidService.addBid(bidCreateDto)){
    ...
    @GetMapping("/{id}")
    public ResponseEntity<BidShowDto> getBidById(@PathVariable int id){
        logger.info("http get request invoked to: '/bid/{id}'");
        return new ResponseEntity<>(bidService.getBidById(id),HttpStatus.OK);
    }
    @PutMapping("/{id}")
    public ResponseEntity<Void> updateBid(@PathVariable int id, @RequestBody @Valid BidCreateDto bidCreateDto, BindingResult bindingResult){
        if (!bindingResult.hasErrors() && bidService.updateBid(id, bidCreateDto)){
